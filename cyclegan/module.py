...
def generator_resnet(image, options, reuse=False, name="generator"):
    with tf.variable_scope(name):
        ...
        def residual_block(x, dim, ks=3, s=1, _name='res'):
            ...
            y = instance_norm(conv2d(y, dim, ks, s, padding='VALID', name=_name + '_c1'), _name + '_bn1', epsilon=1e-5)
            y = tf.pad(tf.nn.relu(y), [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
            y = instance_norm(conv2d(y, dim, ks, s, padding='VALID', name=_name + '_c2'), _name + '_bn2', epsilon=1e-5)
            return y + x
        ...
        c1 = tf.nn.relu(instance_norm(conv2d(c0, options.gf_dim, 7, 1, padding='VALID', name='g_e1_c'), 'g_e1_bn', epsilon=1e-5))
        c2 = tf.nn.relu(instance_norm(conv2d(c1, options.gf_dim * 2, 3, 2, name='g_e2_c'), 'g_e2_bn', epsilon=1e-5))
        c3 = tf.nn.relu(instance_norm(conv2d(c2, options.gf_dim * 4, 3, 2, name='g_e3_c'), 'g_e3_bn', epsilon=1e-5))
        ...
        d1 = tf.nn.relu(instance_norm(d1, 'g_d1_bn', epsilon=1e-5))
        d2 = deconv2d(d1, options.gf_dim, 3, 2, name='g_d2_dc')
        d2 = tf.nn.relu(instance_norm(d2, 'g_d2_bn', epsilon=1e-5))
...
def abs_loss(in_, target, padding="SAME"):
